...
{
  adjLists[src].addFront(dest);
  adjLists[dest].addFront(src);
}
...
template<class T>
void Graph<T>::DFS(int startVertex) 
{
  ...
  for (int i = 0; i < numVertices; i++)
  {
	...
	stack<int> my_stack;
	my_stack.push(startVertex);   //add start to the search
	Node<T>* temp;

  while (!my_stack.empty()) 
  {
    int currVertex = my_stack.top();
    temp=adjLists[currVertex].head;
    my_stack.pop();                   //pull a node
